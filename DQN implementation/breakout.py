...
# 		observation, reward, done, info = env.step([1]) #fire
# 		env.render()
# 		observation, reward, done, info = env.step([2]) #right
# 		env.render()
...
def VideoRecord(env, episode):
	#Video recorder init
	video_recorder = VideoRecorder(env, INFO_GRAPH_WEIGHTS_DIR +'els'+str(episode)+'.mp4', enabled=True)
	if episode%10==1:
...
def main():
	try:
		...
		for episode in range(1,maxrange):
			...
			observation, reward, done, info = env.step(1)  # Do nothing
